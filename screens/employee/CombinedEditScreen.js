...
export default function CombinedEditScreen({ navigation, route }) {
  const { residentId, familiarId } = route.params; // IDs passed from previous screen
  ...
  const fetchResidentData = async () => {
    try {
      ...
      if (response.ok && data.status === 0) {
        ...
        setResidentFechaNacimiento(new Date(resident.fechaNacimiento));
        setResidentGenero(resident.genero || '');
        setResidentTelefono(resident.telefono || '');
        
        if (resident.fotoUrl) {
          setCurrentResidentPhoto(resident.fotoUrl);
          setResidentFotoPreview(resident.fotoUrl);
        }
      } else {
        showNotification('Error al cargar datos del residente', 'error');
      }
    } catch (error) {
      console.error('Error fetching resident data:', error);
      showNotification('Error de conexión al cargar datos del residente', 'error');
    }
  };

  const fetchFamiliarData = async () => {
    try {
      const response = await fetch(`${API_URL}/Familiar/${familiarId}`);
      const data = await response.json();

      if (response.ok && data.status === 0) {
        const familiar = data.data;
        setFamiliarName(familiar.nombre || '');
        setFamiliarApellido(familiar.apellido || '');
        setFamiliarFechaNacimiento(new Date(familiar.fechaNacimiento));
        setFamiliarGenero(familiar.genero || '');
        setFamiliarTelefono(familiar.telefono || '');
        setFamiliarParentesco(familiar.id_parentesco?.toString() || '');
        setFamiliarFirebaseEmail(familiar.email || '');
        setOriginalEmail(familiar.email || '');
      } else {
        showNotification('Error al cargar datos del familiar', 'error');
      }
    } catch (error) {
      console.error('Error fetching familiar data:', error);
      showNotification('Error de conexión al cargar datos del familiar', 'error');
    }
  };
  ...
  const handleCombinedUpdate = async () => {
    ...
    try {
      // Update Firebase user if email or password changed
      if (familiarFirebaseEmail !== originalEmail || familiarFirebasePassword) {
        try {
          const user = auth.currentUser;
          if (user) {
            if (familiarFirebaseEmail !== originalEmail) {
              await updateEmail(user, familiarFirebaseEmail);
            }
            if (familiarFirebasePassword) {
              await updatePassword(user, familiarFirebasePassword);
            }
            
            // Update Firestore document
            await updateDoc(doc(db, "users", user.uid), {
              email: familiarFirebaseEmail,
              updatedAt: new Date().toISOString(),
            });
          }
        } catch (firebaseError) {
          let errorMessage = "Error al actualizar credenciales de Firebase.";
