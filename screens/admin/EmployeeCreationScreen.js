...
export default function EmployeeCreationForm({ onEmployeeCreated, onCancel }) {
  ...
  const handleSaveEmployee = async () => {
    ...
    if (
      ...
      !isValidDateFormat(employeeBirthDate) ||
      !isAdult(employeeBirthDate) ||
      !isValidEmail(employeeEmail) ||
      employeePassword.length < 6 ||
      !doPasswordsMatch(employeePassword, employeeConfirmPassword)
    ) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        let errorMessage =
          errorData.message || errorData.title || 'Error desconocido al guardar datos personales.';
    ...
    <SafeAreaView style={{ flex: 1 }}>
      ...
      >
        <View style={styles.formContainer}>
              ...
              <View style={styles.iosPickerOverlay}>
                ...
                <View style={styles.datePickerWheelContainer}>
                  <Picker
                    ...
                    onValueChange={(itemValue) =>
                      setSelectedDateObject(
                        new Date(itemValue, selectedDateObject.getMonth(), selectedDateObject.getDate())
                      )
                    }
                  ...
                  <Picker
                    ...
                    onValueChange={(itemValue) =>
                      setSelectedDateObject(
                        new Date(selectedDateObject.getFullYear(), itemValue, selectedDateObject.getDate())
                      )
                    }
                    style={styles.datePickerWheel}
                    itemStyle={styles.pickerItem}
                  ...
                  <Picker
                    ...
                    onValueChange={(itemValue) =>
                      setSelectedDateObject(
                        new Date(selectedDateObject.getFullYear(), selectedDateObject.getMonth(), itemValue)
                      )
                    }
                    style={styles.datePickerWheel}
                    itemStyle={styles.pickerItem}
                ...
                <Text style={styles.mockDatePickerText}>
                  (Use un componente nativo para mejor experiencia en móvil.)
                </Text>
              </View>
            )}
          ...
          <TouchableOpacity style={styles.primaryButton} onPress={handleSaveEmployee} disabled={isCreating}>
...
const styles = StyleSheet.create({
  ...
  formContainer: {
    ...containerBaseStyles,
    alignSelf: 'center',
    marginTop: Platform.OS === 'web' ? height * 0.05 : 145,
    marginBottom: 20,
    paddingHorizontal: IS_LARGE_SCREEN ? 30 : 12,
    paddingVertical: 25,
    width: IS_LARGE_SCREEN ? '65%' : '90%',
    maxWidth: IS_LARGE_SCREEN ? 600 : 380,
  },
  formTitle: {
