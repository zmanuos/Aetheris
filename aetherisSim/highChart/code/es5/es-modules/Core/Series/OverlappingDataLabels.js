...
var addEvent = U.addEvent, getAlignFactor = U.getAlignFactor, fireEvent = U.fireEvent, objectEach = U.objectEach, pick = U.pick;
/* *
 *
 *  Functions
 *
    ...
    function getAbsoluteBox(label) {
        ...
        if (label && (!label.alignAttr || label.placed)) {
            var padding = label.box ? 0 : (label.padding || 0), pos = label.alignAttr || {
                x: label.attr('x'),
                y: label.attr('y')
            }, _c = label.getBBox(), height = _c.height, polygon = _c.polygon, width = _c.width, alignOffset = getAlignFactor(label.alignValue) * width;
            label.width = width;
            label.height = height;
            return {
